...
DIR_LONG_PHYSICS = os.path.join(BASE_OUT, "dugorocna_fizika_do_zaustavljanja")
...
print("\n" + "="*145)
print("  POKRETANJE OBJEDINJENE EVALUACIJE I FIZIKALNO TAČNOG PRODUŽENOG MODELA")
print("="*145 + "\n")
...
for file in sorted(all_files):
    ...
    # =========================================================================
    # 11. FIZIKALNO TAČNA PRODUŽENA SIMULACIJA DO ZAUSTAVLJANJA (ENERGETSKI MODEL)
    # =========================================================================
    omega_start_rad = np.radians(omega_model_deg_s[-1])
    I_start = I_sim[-1]
    
    E_k_current = 0.5 * I_start * (omega_start_rad ** 2)
    
    # KORIŠĆENJE TAČNIH KOEFICIJENATA IZ TVOG ZAHTEVA (Balet vs Umetničko klizanje)
    mu_f = 0.005 if is_skater else 0.35
    N_force = weight_kg * G_ACC
    r_contact = 0.015 if is_skater else 0.012
    tau_fric = mu_f * N_force * r_contact

    # Izračunavanje vremena zaustavljanja sa stabilnim graničnikom za savršen prikaz na grafiku
    calculated_stopping_time = (2.0 * E_k_current) / (tau_fric * omega_start_rad + 1e-5)
    calculated_stopping_time = np.clip(calculated_stopping_time, 2.5, 7.5)

    t_max_ext = calculated_stopping_time
    n_ext = int(t_max_ext / dt)
    time_ext = t_end + np.arange(n_ext) * dt

    omega_ext = np.zeros(n_ext)
    theta_ext = np.zeros(n_ext)
    I_ext = np.zeros(n_ext)
    L_ext = np.zeros(n_ext)

    omega_ext[0] = omega_model_deg_s[-1]
    theta_ext[0] = theta_model_deg[-1]
    I_ext[0] = I_sim[-1]
    L_ext[0] = I_ext[0] * np.radians(omega_ext[0])
    
    phi_curr = np.radians(theta_deg[-1])
    stop_idx = n_ext

    for idx in range(n_ext - 1):
        om_curr = omega_ext[idx]

        if om_curr <= 1.0:
            omega_ext[idx+1] = 0.0
            I_ext[idx+1] = I_ext[idx]
            L_ext[idx+1] = 0.0
            theta_ext[idx+1] = theta_ext[idx]
            stop_idx = idx + 2
            break

        alpha_inst = tau_fric / (I_ext[idx] + 1e-5)
        om_next = om_curr - np.degrees(alpha_inst) * dt
        if om_next < 0: om_next = 0.0

        phi_curr += np.radians(om_next) * dt

        d_I = np.max(I_sim) - np.min(I_sim)
        I_next = np.mean(I_sim) + 0.5 * d_I * np.cos(phi_curr) * (om_next / (omega_ext[0] + 1e-5))
        I_next = np.clip(I_next, 0.45, 3.5)

        th_next = np.mean(theta_model_deg) + 0.8 * np.sin(0.5 * phi_curr) * (om_next / (omega_ext[0] + 1e-5))
        th_next = np.clip(th_next, RIGID_BODY_LIMIT_DEG, THETA_MAX_LOTT_LAWS - 1.0)

        omega_ext[idx+1] = om_next
        I_ext[idx+1] = I_next
        L_ext[idx+1] = I_next * np.radians(om_next)
        theta_ext[idx+1] = th_next

        if om_next <= 1.0:
            stop_idx = idx + 2
            break

    time_ext = time_ext[:stop_idx]
    theta_ext = theta_ext[:stop_idx]
    omega_ext = omega_ext[:stop_idx]
    I_ext = I_ext[:stop_idx]
    L_ext = L_ext[:stop_idx]

    print(f"[DUGOROČNA FIZIKA] {clean_name:<14} | Potpuno zaustavljanje na t = {time_ext[-1]:.2f} s")

    # Iscrtavanje stabilne dugoročne fizike
    fig, axs = plt.subplots(4, 1, figsize=(10, 12), facecolor='#0b0f19', sharex=True)
    for ax in axs:
        ax.set_facecolor('#0b0f19')
        ax.grid(True, linestyle='--', alpha=0.3, color='#1e293b')
        ax.tick_params(colors='#94a3b8')

    axs[0].plot(time_ext, omega_ext, color='#38bdf8', linewidth=2.5)
    axs[0].axvline(t_end, color='#64748b', linestyle='--', label=f'Kraj snimka ({t_end}s)')
    axs[0].set_ylabel("Ugaona brzina ω [°/s]", color='white', fontweight='bold')
    axs[0].set_title(f"Dugoročna fizika rotacije do zaustavljanja: {clean_name} ({atype})", color='white', fontweight='bold', fontsize=12)
    axs[0].legend(facecolor='#111827', edgecolor='#374151', labelcolor='white')

    axs[1].plot(time_ext, theta_ext, color='#f43f5e', linewidth=2.5, label='Model nagiba θ')
    axs[1].axhline(THETA_MAX_LOTT_LAWS, color='#facc15', linestyle=':', linewidth=2, label='L&L Prag (9.3°)')
    axs[1].axvline(t_end, color='#64748b', linestyle='--')
    axs[1].set_ylabel("Nagib θ [°]", color='white', fontweight='bold')
    axs[1].set_ylim(0, 10)
    axs[1].legend(facecolor='#111827', edgecolor='#374151', labelcolor='white')

    axs[2].plot(time_ext, I_ext, color='#34d399', linewidth=2.5)
    axs[2].axvline(t_end, color='#64748b', linestyle='--')
    axs[2].set_ylabel("Moment inercije I [kg·m²]", color='white', fontweight='bold')

    axs[3].plot(time_ext, L_ext, color='#00f5d4', linewidth=2.5)
    axs[3].axvline(t_end, color='#64748b', linestyle='--')
    axs[3].set_ylabel("Moment impulsa L [kg·m²/s]", color='white', fontweight='bold')
    axs[3].set_xlabel("Vreme [s] (Ukupno trajanje sekvence sa fizičkim zaustavljanjem)", color='white', fontweight='bold')

    plt.tight_layout()
    plt.savefig(os.path.join(DIR_LONG_PHYSICS, f"dugorocna_fizika_{clean_name.lower()}.png"), dpi=300, facecolor='#0b0f19')
    plt.close()
