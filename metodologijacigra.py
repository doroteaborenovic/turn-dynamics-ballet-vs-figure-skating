...
# -------------------------------------------------------------------------
# EGZAKTNO REŠAVANJE ČIGRE KOVALJEVSKE SA TRENJEM I KOREKTNIM SKALIRANJEM
# -------------------------------------------------------------------------

def solve_kovalevskaya_exact_friction(theta_0_rad, omega_0_rad_s, J_phys, m_kg, l_com, mu_friction, r_contact, time_eval):
    # Fizička karakteristična frekvencija i vremenska skala
    omega_unit = np.sqrt((m_kg * G_ACC * l_com) / max(1e-5, J_phys))
    tau = 1.0 / omega_unit

    # Proračun bezdimenzionog koeficijenta trenja beta
    tau_fric = mu_friction * m_kg * G_ACC * r_contact
    beta_phys = tau_fric / max(0.1, (J_phys * omega_0_rad_s))
    beta = beta_phys * tau

    # Vremenska osa preslikana u bezdimenziono vreme
    t_eval_dimless = time_eval / tau
    ...
    m1_0 = 0.0
    m2_0 = 0.0
    # Bezdimenziona komponenta impulsa m3_0
    m3_0 = 0.5 * (omega_0_rad_s / omega_unit)
    ...
    sol = solve_ivp(
        deriv, 
        (t_eval_dimless[0], t_eval_dimless[-1]), 
        y0, 
        t_eval=t_eval_dimless, 
        events=fall_event, 
        method='RK45', 
        rtol=1e-7, 
        atol=1e-7
    ...
    # Vraćanje rešenja u realne fizičke jedinice
    t_sol = sol.t * tau
    m1 = sol.y[0]
    m2 = sol.y[1]
    m3 = sol.y[2]
    ...
    omega_top_rad_s_raw = omega_unit * np.sqrt(m1**2 + m2**2 + (2.0 * m3)**2)
    ...
    E_k_top_raw = 0.5 * J_phys * (omega_unit**2) * (m1**2 + m2**2 + 2.0 * (m3**2))

    if len(sol.t_events[0]) > 0:
        has_fallen = True
        t_fall = float(sol.t_events[0][0] * tau)
...
for file in sorted(all_files):
    ...
    l_com_val = 0.56 * height_m
    h_com = np.full(n_frames, l_com_val)
