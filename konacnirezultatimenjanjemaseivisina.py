...
# Parametarska mreža visina i masa
HEIGHT_GRID = np.round(np.arange(1.58, 1.79, 0.02), 2)  # [1.58, 1.60, ..., 1.78] m
WEIGHT_GRID = np.round(np.arange(42.0, 55.0, 2.0), 1)   # [42, 44, ..., 54] kg
...
DIR_INER_SENZ       = os.path.join(BASE_OUT, "grafici_inercija_senzitivnost_matrica")
...
DIR_KIN_OMEGA_SENZ  = os.path.join(BASE_OUT, "grafici_ugaona_brzina_senzitivnost_matrica")
DIR_TORQUE_SENZ     = os.path.join(BASE_OUT, "grafici_moment_sile_senzitivnost_matrica")
...
ALL_DIRS = [
    ...
    DIR_INER_INDIVIDUAL, DIR_INER_SENZ, DIR_INER_SUMMARY,
    DIR_KIN_OMEGA, DIR_KIN_OMEGA_SENZ, DIR_TORQUE_SENZ, DIR_KIN_ALPHA,
...
COLORS_9 = [
    "#00f5d4", "#ff007f", "#fee440", 
    "#7b2cbf", "#00b4d8", "#f77f00", 
    "#52b788", "#e0aaff", "#ff5400"
]
...
# =============================================================================
# 3. GLAVNA OBRADA (SVE KOMPONENTE U JEDNOM PROLAZU)
# =============================================================================
...
print("\n" + "="*125)
print("  POKRETANJE OBJEDINJENE ANALIZE: KINEMATIKA, MOMENT INERCIJE, MOMENT SILE, STABILNOST")
print("="*125 + "\n")
...
for file in sorted(all_files):
    ...
    # Pojedinačni grafik Momenta Inercije (Standardni)
    ...
    # Dvostruki grafik: Ugaona brzina (Standardni)
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15.5, 6.5), facecolor='#0b0f19')
    for ax in (ax1, ax2):
        ax.set_facecolor('#0b0f19')
        ax.grid(True, linestyle='--', alpha=0.35, color='#1e293b')
    ...
    # =========================================================================
    # E) DVOSTRUKI SENZITIVNI GRAFICI: INERCIJA, BRZINA I MOMENT SILE (T)
    #    (IDENTIČAN FORMAT: 9 LINIJA LEVO + 2D MATRICA DESNO)
    # =========================================================================
    curves_IB_9 = {}
    curves_Omega_9 = {}
    curves_Torque_9 = {}
    ...
    for sim_h in HEIGHT_GRID:
        scale_sim = sim_h / med_h
        pts_sim = pts_array * scale_sim
        pts_sim[:, :, 2] = pts_sim[:, :, 2] * 0.50

        mid_sh_s = (pts_sim[:, 11, :] + pts_sim[:, 12, :]) / 2.0
        mid_hp_s = (pts_sim[:, 23, :] + pts_sim[:, 24, :]) / 2.0
        head_v_s = pts_sim[:, 0, :] + 0.5 * (pts_sim[:, 0, :] - mid_sh_s)

        seg_endpoints_s = {
            "Head":       (mid_sh_s, head_v_s),
            "Trunk":      (mid_sh_s, mid_hp_s),
            "R_UpperArm": (pts_sim[:, 12, :], pts_sim[:, 14, :]),
            "L_UpperArm": (pts_sim[:, 11, :], pts_sim[:, 13, :]),
            "R_Forearm":  (pts_sim[:, 14, :], pts_sim[:, 16, :]),
            "L_Forearm":  (pts_sim[:, 13, :], pts_sim[:, 15, :]),
            "R_Hand":     (pts_sim[:, 16, :], pts_sim[:, 16, :] + 0.05 * (pts_sim[:, 16, :] - pts_sim[:, 14, :])),
            "L_Hand":     (pts_sim[:, 15, :], pts_sim[:, 15, :] + 0.05 * (pts_sim[:, 15, :] - pts_sim[:, 13, :])),
            "R_Thigh":    (pts_sim[:, 24, :], pts_sim[:, 26, :]),
            "L_Thigh":    (pts_sim[:, 23, :], pts_sim[:, 25, :]),
            "R_Shank":    (pts_sim[:, 26, :], pts_sim[:, 28, :]),
            "L_Shank":    (pts_sim[:, 25, :], pts_sim[:, 27, :]),
            "R_Foot":     (pts_sim[:, 30, :], pts_sim[:, 32, :]),
            "L_Foot":     (pts_sim[:, 29, :], pts_sim[:, 31, :])
        }
        seg_s = {k: interp_seg(p1, p2, DE_LEVA_FEMALE[k]["pos"]) for k, (p1, p2) in seg_endpoints_s.items()}

        axis_x_s = savgol_filter(0.5 * (pts_sim[:, p_ank, 0] + mid_hp_s[:, 0]), win_smooth, 2)
        axis_z_s = savgol_filter(0.5 * (pts_sim[:, p_ank, 2] + mid_hp_s[:, 2]), win_smooth, 2)

        r_sq_s = {}
        for s_n, s_c in seg_s.items():
            if s_n not in support_leg_names:
                r_val = np.sqrt((s_c[:, 0] - axis_x_s)**2 + (s_c[:, 2] - axis_z_s)**2)
                r_val = np.clip(r_val, 0.08, 0.96 * (sim_h / 1.65))
                r_sq_s[s_n] = r_val ** 2

        for sim_w in WEIGHT_GRID:
            I_B_sim = np.zeros(n_frames)
            for s_n, r_sq in r_sq_s.items():
                I_B_sim += (DE_LEVA_FEMALE[s_n]["mass"] * sim_w) * r_sq

            I_B_sim = np.maximum(savgol_filter(I_B_sim, window_length=win_dyn_iner, polyorder=2), 0.40)
            dI_B_sim_dt = np.clip(savgol_filter(I_B_sim, window_length=win_dyn_iner, polyorder=2, deriv=1, delta=dt), -6.0, 6.0)

            # Moment sile: T(t) = I_B * alpha + (dI/dt) * omega
            T_sim = I_B_sim * alpha_B + dI_B_sim_dt * omega_B
            T_sim = savgol_filter(T_sim, window_length=win_dyn_iner, polyorder=2)
            
            omega_actual = omega_deg_s

            key_tag = f"H={sim_h:.2f}m, W={sim_w:.0f}kg"
            if (sim_h in [1.58, 1.68, 1.78]) and (sim_w in [42.0, 48.0, 54.0]):
                curves_IB_9[key_tag] = I_B_sim
                curves_Omega_9[key_tag] = omega_actual
                curves_Torque_9[key_tag] = T_sim

            grid_records.append({
                "Visina_m": sim_h,
                "Masa_kg": sim_w,
                "I_B_sr": np.mean(I_B_sim),
                "Omega_sr": np.mean(omega_actual),
                "T_max": np.max(np.abs(T_sim))
            })

    df_grid = pd.DataFrame(grid_records)
    pivot_IB_ath = df_grid.pivot_table(index="Visina_m", columns="Masa_kg", values="I_B_sr")
    pivot_Omega_ath = df_grid.pivot_table(index="Visina_m", columns="Masa_kg", values="Omega_sr")
    pivot_Torque_ath = df_grid.pivot_table(index="Visina_m", columns="Masa_kg", values="T_max")

    # 1. SENZITIVNOST MOMENTA INERCIJE
    fig_ib, (ax_ib1, ax_ib2) = plt.subplots(1, 2, figsize=(17.0, 7.0), facecolor='#0b0f19')
    for ax in (ax_ib1, ax_ib2):
        ax.set_facecolor('#0b0f19')
        ax.grid(True, linestyle='--', alpha=0.35, color='#1e293b')
        ax.tick_params(colors='#94a3b8', labelsize=10)

    for idx, (lbl, c_data) in enumerate(curves_IB_9.items()):
        ax_ib1.plot(time_axis, c_data, linewidth=2.2, color=COLORS_9[idx % len(COLORS_9)], label=lbl)

    ax_ib1.set_xlabel("Vreme rotacije [s]", fontsize=11, fontweight='bold', color='#94a3b8')
    ax_ib1.set_ylabel("Moment inercije I_B [kg*m²]", fontsize=11, fontweight='bold', color='#94a3b8')
    ax_ib1.set_title(f"Dinamika I_B(t) kroz varijacije visine i mase\nOriginal: {height_m:.2f}m / {weight_kg:.1f}kg", fontsize=12, fontweight='bold', color='#ffffff', pad=12)
    ax_ib1.set_xlim(0, time_axis[-1])
    ax_ib1.legend(loc='upper right', facecolor='#111827', edgecolor='#374151', labelcolor='white', fontsize=8.5, ncol=2)

    piv_ib_sorted = pivot_IB_ath.sort_index(ascending=False)
    im_ib = ax_ib2.imshow(piv_ib_sorted.values, cmap='plasma', aspect='auto')
    ax_ib2.set_xticks(np.arange(len(piv_ib_sorted.columns)))
    ax_ib2.set_xticklabels([f"{int(c)}kg" for c in piv_ib_sorted.columns], color='#94a3b8')
    ax_ib2.set_yticks(np.arange(len(piv_ib_sorted.index)))
    ax_ib2.set_yticklabels([f"{r:.2f}m" for r in piv_ib_sorted.index], color='#94a3b8')
    ax_ib2.set_xlabel("Masa tela [kg]", fontsize=11, fontweight='bold', color='#94a3b8')
    ax_ib2.set_ylabel("Visina tela [m]", fontsize=11, fontweight='bold', color='#94a3b8')
    ax_ib2.set_title("2D Matrica senzitivnosti: Srednji I_B [kg*m²]\nKvadratna zavisnost od H i linearna od W", fontsize=12, fontweight='bold', color='#ffffff', pad=12)

    for r_i in range(len(piv_ib_sorted.index)):
        for c_j in range(len(piv_ib_sorted.columns)):
            ax_ib2.text(c_j, r_i, f"{piv_ib_sorted.values[r_i, c_j]:.2f}", ha='center', va='center', color='white', fontweight='bold', fontsize=8.5)

    cbar_ib = plt.colorbar(im_ib, ax=ax_ib2, fraction=0.046, pad=0.04)
    cbar_ib.ax.yaxis.set_tick_params(color='#ffffff')
    plt.setp(plt.getp(cbar_ib.ax.axes, 'yticklabels'), color='#ffffff')

    plt.suptitle(f"SENZITIVNOST MOMENTA INERCIJE: {clean_name} ({atype})", fontsize=14, fontweight='bold', color='#ffffff', y=0.98)
    plt.tight_layout()
    plt.savefig(os.path.join(DIR_INER_SENZ, f"inercija_senzitivnost_{clean_name.lower()}.png"), dpi=300, facecolor='#0b0f19')
    plt.close()

    # 2. SENZITIVNOST UGAONE BRZINE
    fig_w, (ax_w1, ax_w2) = plt.subplots(1, 2, figsize=(17.0, 7.0), facecolor='#0b0f19')
    for ax in (ax_w1, ax_w2):
        ax.set_facecolor('#0b0f19')
        ax.grid(True, linestyle='--', alpha=0.35, color='#1e293b')
        ax.tick_params(colors='#94a3b8', labelsize=10)

    ax_w1.plot(time_axis, omega_deg_s, color='#ff007f', linewidth=2.8, label=f'Izmerena ω(t) [{clean_name}]')
    ax_w1.axhline(np.mean(omega_deg_s), color='#00f5d4', linestyle='--', linewidth=1.8, label=f'Srednja ω: {np.mean(omega_deg_s):.1f} °/s')
    ax_w1.axhline(np.max(omega_deg_s), color='#fee440', linestyle=':', linewidth=1.6, label=f'Maksimalna ω: {np.max(omega_deg_s):.1f} °/s')
    ax_w1.fill_between(time_axis, 0, omega_deg_s, color='#ff007f', alpha=0.15)

    ax_w1.set_xlabel("Vreme rotacije [s]", fontsize=11, fontweight='bold', color='#94a3b8')
    ax_w1.set_ylabel("Ugaona brzina ω [°/s]", fontsize=11, fontweight='bold', color='#94a3b8')
    ax_w1.set_title(f"Kinematički profil ugaone brzine iz snimka ({t_start}s–{t_end}s)\nStvarni pokret sportistkinje (Ukupno okreta: {total_rotations:.2f})", fontsize=12, fontweight='bold', color='#ffffff', pad=12)
    ax_w1.set_xlim(0, time_axis[-1])
    ax_w1.legend(loc='upper right', facecolor='#111827', edgecolor='#374151', labelcolor='white', fontsize=9.0)

    piv_w_sorted = pivot_Omega_ath.sort_index(ascending=False)
    im_w = ax_w2.imshow(piv_w_sorted.values, cmap='plasma', aspect='auto')
    ax_w2.set_xticks(np.arange(len(piv_w_sorted.columns)))
    ax_w2.set_xticklabels([f"{int(c)}kg" for c in piv_w_sorted.columns], color='#94a3b8')
    ax_w2.set_yticks(np.arange(len(piv_w_sorted.index)))
    ax_w2.set_yticklabels([f"{r:.2f}m" for r in piv_w_sorted.index], color='#94a3b8')
    ax_w2.set_xlabel("Masa tela [kg]", fontsize=11, fontweight='bold', color='#94a3b8')
    ax_w2.set_ylabel("Visina tela [m]", fontsize=11, fontweight='bold', color='#94a3b8')
    ax_w2.set_title(f"2D Matrica: Srednja ugaona brzina ω = {np.mean(omega_deg_s):.1f} [°/s]\nKinematička nepromenljivost snimljenog pokreta", fontsize=12, fontweight='bold', color='#ffffff', pad=12)

    for r_i in range(len(piv_w_sorted.index)):
        for c_j in range(len(piv_w_sorted.columns)):
            ax_w2.text(c_j, r_i, f"{piv_w_sorted.values[r_i, c_j]:.1f}", ha='center', va='center', color='white', fontweight='bold', fontsize=8.5)

    cbar_w = plt.colorbar(im_w, ax=ax_w2, fraction=0.046, pad=0.04)
    cbar_w.ax.yaxis.set_tick_params(color='#ffffff')
    plt.setp(plt.getp(cbar_w.ax.axes, 'yticklabels'), color='#ffffff')

    plt.suptitle(f"UGAONA BRZINA ROTACIJE: {clean_name} ({atype})", fontsize=14, fontweight='bold', color='#ffffff', y=0.98)
    plt.tight_layout()
    plt.savefig(os.path.join(DIR_KIN_OMEGA_SENZ, f"brzina_senzitivnost_{clean_name.lower()}.png"), dpi=300, facecolor='#0b0f19')
    plt.close()

    # 3. SENZITIVNOST MOMENTA SILE T(t) (UVIJANJE TRUPA)
    fig_t, (ax_t1, ax_t2) = plt.subplots(1, 2, figsize=(17.0, 7.0), facecolor='#0b0f19')
    for ax in (ax_t1, ax_t2):
        ax.set_facecolor('#0b0f19')
        ax.grid(True, linestyle='--', alpha=0.35, color='#1e293b')
        ax.tick_params(colors='#94a3b8', labelsize=10)

    for idx, (lbl, c_data) in enumerate(curves_Torque_9.items()):
        ax_t1.plot(time_axis, c_data, linewidth=2.2, color=COLORS_9[idx % len(COLORS_9)], label=lbl)

    ax_t1.axhline(0, color='#64748b', linestyle=':', linewidth=1.2)
    ax_t1.set_xlabel("Vreme rotacije [s]", fontsize=11, fontweight='bold', color='#94a3b8')
    ax_t1.set_ylabel("Moment sile uvijanja T [Nm]", fontsize=11, fontweight='bold', color='#94a3b8')
    ax_t1.set_title(f"Dinamika T(t) = I_B·α + (dI/dt)·ω kroz varijacije H i W\nPotreban mišićni obrtni moment za isti pokret", fontsize=12, fontweight='bold', color='#ffffff', pad=12)
    ax_t1.set_xlim(0, time_axis[-1])
    ax_t1.legend(loc='upper right', facecolor='#111827', edgecolor='#374151', labelcolor='white', fontsize=8.5, ncol=2)

    piv_t_sorted = pivot_Torque_ath.sort_index(ascending=False)
    im_t = ax_t2.imshow(piv_t_sorted.values, cmap='plasma', aspect='auto')
    ax_t2.set_xticks(np.arange(len(piv_t_sorted.columns)))
    ax_t2.set_xticklabels([f"{int(c)}kg" for c in piv_t_sorted.columns], color='#94a3b8')
    ax_t2.set_yticks(np.arange(len(piv_t_sorted.index)))
    ax_t2.set_yticklabels([f"{r:.2f}m" for r in piv_t_sorted.index], color='#94a3b8')
    ax_t2.set_xlabel("Masa tela [kg]", fontsize=11, fontweight='bold', color='#94a3b8')
    ax_t2.set_ylabel("Visina tela [m]", fontsize=11, fontweight='bold', color='#94a3b8')
    ax_t2.set_title("2D Matrica senzitivnosti: Maksimalni moment sile |T| [Nm]\nSkalira sa (Masa × Visina²)", fontsize=12, fontweight='bold', color='#ffffff', pad=12)

    for r_i in range(len(piv_t_sorted.index)):
        for c_j in range(len(piv_t_sorted.columns)):
            ax_t2.text(c_j, r_i, f"{piv_t_sorted.values[r_i, c_j]:.1f}", ha='center', va='center', color='white', fontweight='bold', fontsize=8.5)

    cbar_t = plt.colorbar(im_t, ax=ax_t2, fraction=0.046, pad=0.04)
    cbar_t.ax.yaxis.set_tick_params(color='#ffffff')
    plt.setp(plt.getp(cbar_t.ax.axes, 'yticklabels'), color='#ffffff')

    plt.suptitle(f"SENZITIVNOST MOMENTA SILE UVIJANJA: {clean_name} ({atype})", fontsize=14, fontweight='bold', color='#ffffff', y=0.98)
    plt.tight_layout()
    plt.savefig(os.path.join(DIR_TORQUE_SENZ, f"moment_sile_senzitivnost_{clean_name.lower()}.png"), dpi=300, facecolor='#0b0f19')
    plt.close()

    print(f"[OBRADA ZAVRŠENA] {clean_name:<14} | {atype:<18} | Okreta: {total_rotations:4.2f} | θ sr: {np.mean(theta_topple_deg):4.2f}° | I_B sr: {np.mean(I_B):.2f} kg*m² | ω sr: {np.mean(omega_deg_s):5.1f} °/s")
...
print(f"  ├── Senzitivnost inercije (Matrica + 9 lin):     {DIR_INER_SENZ}/")
print(f"  ├── Zbirni grafici inercije:                     {DIR_INER_SUMMARY}/")
print(f"  ├── Grafici ugaone brzine (ω):                   {DIR_KIN_OMEGA}/")
print(f"  ├── Senzitivnost brzine (Matrica + Profil):      {DIR_KIN_OMEGA_SENZ}/")
print(f"  ├── Senzitivnost momenta sile (Matrica + 9 lin): {DIR_TORQUE_SENZ}/")
