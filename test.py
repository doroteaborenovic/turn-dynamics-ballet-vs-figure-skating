...
# =============================================================================
# 1. PARAMETRI, BAZA ATLETA, DE LEVA (1996) I DIREKTORIJUMI
# =============================================================================
...
G_ACC = 9.81
THETA_MAX_LOTT_LAWS = 9.3   # Prag pada (Lott & Laws 2012) [°]
THETA_INITIAL_TOP_DEG = 1.5 # Početni nagib [°]
RIGID_BODY_LIMIT_DEG = 1.0
T_SIMULATION_LONG = 300.0   # 5 MINUTA (300 sekundi)
...
DIR_LOTT_TEST     = os.path.join(BASE_OUT, "grafici_lott_laws_eksperiment_test")
DIR_TABLES        = os.path.join(BASE_OUT, "tabele_rezultati")

for d in [BASE_OUT, DIR_MASTER_5PANEL, DIR_LONG_SIM, DIR_LOTT_TEST, DIR_TABLES]:
...
def clean_and_interpolate_signal(arr, vis=None, vis_threshold=0.35):
    ...
    if speed_thresh > 0.04: arr[diff > speed_thresh] = np.nan
    ...
    if win >= 5: filled = savgol_filter(filled, window_length=win, polyorder=2)
    return filled
...
# =============================================================================
# 3. ČIGRA KOVALJEVSKE (PERELOMOV 2001) SA DODATIM TRENJEM
# =============================================================================

def solve_kovalevskaya_exact_friction(theta_0_rad, omega_0_rad_s, J_phys, m_kg, mu_friction, r_contact, time_eval, theta_limit_deg=THETA_MAX_LOTT_LAWS):
    ...
    def fall_event(t, y):
        ...
        return th - np.radians(theta_limit_deg)
...
# =============================================================================
# 4. GLAVNA OBRADA I SIMULACIJA (4s POREĐENJE + 300s DUGOTRAJNI TEST + LOTT&LAWS BENCHMARK)
# =============================================================================

all_files = glob.glob(os.path.join(INPUT_DIR, "*_kinematics.csv"))
if not all_files: all_files = glob.glob(os.path.join(INPUT_DIR, "*.csv"))
all_files = [f for f in all_files if "SUMARNA" not in f and "popravljeno" not in f and "fizika_" not in f]

master_rows_4s = []
master_rows_300s = []
master_rows_lott_benchmark = []
long_sim_data = {}
processed_names = set()

print("\n" + "="*145)
print(f"  GLAVNA NAUČNA EVALUACIJA: ČIGRA KOVALJEVSKE SA TRENJEM + EKSPERIMENTALNI PROTOKOL LOTT & LAWS (2012)")
print("="*145 + "\n")
...
for file in sorted(all_files):
    ...
    omega_0_rev_s = omega_0_rad_s / (2.0 * np.pi)
    ...
    # -------------------------------------------------------------------------
    # 8. EKSPERIMENTALNI PROTOKOL LOTT & LAWS (2012) NA TVOJEM MODELU ČIGRE
    # -------------------------------------------------------------------------
    t_eval_bench = np.linspace(0.0, 4.0, 1000)
    
    # Za theta_i = 0.1 deg
    _, _, _, _, _, t_fall_01, turns_01, _, _, _, _, _, _ = solve_kovalevskaya_exact_friction(
        theta_0_rad=np.radians(0.1), omega_0_rad_s=omega_0_rad_s, J_phys=J_phys_0,
        m_kg=weight_kg, mu_friction=mu_val, r_contact=r_eff_val, time_eval=t_eval_bench, theta_limit_deg=THETA_MAX_LOTT_LAWS
    )
    # Za theta_i = 1.0 deg
    _, _, _, _, _, t_fall_10, turns_10, _, _, _, _, _, _ = solve_kovalevskaya_exact_friction(
        theta_0_rad=np.radians(1.0), omega_0_rad_s=omega_0_rad_s, J_phys=J_phys_0,
        m_kg=weight_kg, mu_friction=mu_val, r_contact=r_eff_val, time_eval=t_eval_bench, theta_limit_deg=THETA_MAX_LOTT_LAWS
    )
    # Za theta_i = 3.0 deg
    _, _, _, _, _, t_fall_30, turns_30, _, _, _, _, _, _ = solve_kovalevskaya_exact_friction(
        theta_0_rad=np.radians(3.0), omega_0_rad_s=omega_0_rad_s, J_phys=J_phys_0,
        m_kg=weight_kg, mu_friction=mu_val, r_contact=r_eff_val, time_eval=t_eval_bench, theta_limit_deg=THETA_MAX_LOTT_LAWS
    )
    # Za izmereni theta_i
    _, _, _, _, _, t_fall_meas, turns_meas, _, _, _, _, _, _ = solve_kovalevskaya_exact_friction(
        theta_0_rad=np.radians(theta_i_meas), omega_0_rad_s=omega_0_rad_s, J_phys=J_phys_0,
        m_kg=weight_kg, mu_friction=mu_val, r_contact=r_eff_val, time_eval=t_eval_bench, theta_limit_deg=THETA_MAX_LOTT_LAWS
    )

    master_rows_lott_benchmark.append({
        "Atleta": clean_name, "Grupa": atype, "Izmeren theta_i [°]": round(theta_i_meas, 1),
        "Spin rate [rev/s]": round(omega_0_rev_s, 2),
        "Realno (Video) [okr]": round(total_rotations_4s, 2),
        "Čigra (izmeren th_i) [okr]": round(turns_meas, 2),
        "Čigra (th_i = 0.1°) [okr]": round(turns_01, 2),
        "Čigra (th_i = 1.0°) [okr]": round(turns_10, 2),
        "Čigra (th_i = 3.0°) [okr]": round(turns_30, 2),
        "Lott&Laws Ref (th_i=0.1°) [okr]": "3.3 ± 0.2",
        "Lott&Laws Ref (th_i=1.0°) [okr]": "1.8 ± 0.2",
        "Lott&Laws Ref (th_i=3.0°) [okr]": "1.1 ± 0.1"
    })

    # -------------------------------------------------------------------------
    # 9. EGZAKTNO REŠAVANJE ČIGRE (4 SEKUNDE PROZOR)
    # -------------------------------------------------------------------------
    theta_deg_top, omega_deg_s_top, alpha_deg_s2_top, L_top, E_k_top, t_fall_4s, turns_fall_4s, beta_calc, delta_E_k_4s, has_fallen_4s, _, _, _ = solve_kovalevskaya_exact_friction(
        ...
        r_contact=r_eff_val,
        time_eval=time_axis,
        theta_limit_deg=THETA_MAX_LOTT_LAWS
    )

    # -------------------------------------------------------------------------
    # 10. DUGOTRAJNA SIMULACIJA ČIGRE (300 SEKUNDI / 5 MINUTA)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 11. GRAFIK: 5-PANEL POREĐENJE SA OSOBOM (0–4s)
    # -------------------------------------------------------------------------
    ...
    print(f"[OBRAĐENO] {clean_name:<14} | {atype:<18} | Realno: {total_rotations_4s:4.2f} okr | Čigra (th_i={theta_i_meas:.1f}°): {turns_meas:.2f} okr | 5min test: {vreme_300s_str}")

# =============================================================================
# 5. REPRODUKCIJA SLIKE 4 (FIGURE 4) POMOĆU ČIGRE KOVALJEVSKE
# =============================================================================

plt.figure(figsize=(10, 7), dpi=300, facecolor='#ffffff')
ax_f4 = plt.gca()
ax_f4.set_facecolor('#ffffff')
ax_f4.grid(True, linestyle='-', alpha=0.5, color='#cccccc')
ax_f4.tick_params(colors='#000000', labelsize=11)

t_eval_f4 = np.linspace(0.0, 3.0, 300)

omega_ref_rad = 1.7 * 2.0 * np.pi

markers_dict = {0.1: ('o', 'Theta_i = 0.1 deg'), 1.0: ('s', 'Theta_i = 1 deg'), 3.0: ('^', 'Theta_i = 3 deg')}

for th_init, (mkr, lbl_suffix) in markers_dict.items():
    # Ženski model
    _, _, _, _, _, _, _, _, _, _, t_f, th_f, _ = solve_kovalevskaya_exact_friction(
        theta_0_rad=np.radians(th_init), omega_0_rad_s=omega_ref_rad, J_phys=0.534,
        m_kg=45.0, mu_friction=0.30, r_contact=0.025, time_eval=t_eval_f4, theta_limit_deg=90.0
    )
    # Muški model
    _, _, _, _, _, _, _, _, _, _, t_m, th_m, _ = solve_kovalevskaya_exact_friction(
        theta_0_rad=np.radians(th_init), omega_0_rad_s=omega_ref_rad, J_phys=1.340,
        m_kg=77.0, mu_friction=0.30, r_contact=0.025, time_eval=t_eval_f4, theta_limit_deg=90.0
    )
    
    ax_f4.plot(t_f, th_f, color='black', marker=mkr, markevery=8, markersize=5, linewidth=1.8, label=f'Female, {lbl_suffix}')
    ax_f4.plot(t_m, th_m, color='grey', marker=mkr, markevery=8, markersize=5, linewidth=1.8, label=f'Male, {lbl_suffix}')

ax_f4.set_xlabel("time (s)", fontsize=12, fontweight='bold', color='#000000')
ax_f4.set_ylabel(r"$\theta$ (degrees)", fontsize=12, fontweight='bold', color='#000000')
ax_f4.set_title(r"Topple Angle, $\theta$ , as a function of time" + "\n" + "(Reprodukcija Figure 4 Lott & Laws pomoću modela Čigre Kovaljevske)", fontsize=13, fontweight='bold', color='#000000', pad=12)
ax_f4.set_xlim(0, 3.0)
ax_f4.set_ylim(0, 90.0)
ax_f4.legend(loc='upper left', facecolor='#ffffff', edgecolor='#000000', labelcolor='black', fontsize=9.5)
plt.tight_layout()
plt.savefig(os.path.join(DIR_LOTT_TEST, "reprodukcija_figure_4_lott_laws_cigra.png"), dpi=300)
plt.close()

# =============================================================================
# 6. DUGOTRAJNI ZBIRNI GRAFICI (300 SEKUNDI / 5 MINUTA)
# =============================================================================
...
df_master_lott_bench = pd.DataFrame(master_rows_lott_benchmark)
df_master_lott_bench.to_csv(os.path.join(DIR_TABLES, "tabela_lott_laws_2012_benchmark_poređenje.csv"), index=False)
...
print("\n" + "="*165)
print("  TABELA 2: POREĐENJE SA EKSPERIMENTALNIM PROTOKOLOM LOTT & LAWS (2012) — BROJ OKRETA DO PADA (θ < 9.3°)")
print("="*165)
print(df_master_lott_bench.to_string(index=False))

print("\n" + "="*165)
print(f"  TABELA 3: DUGOTRAJNA SIMULACIJA ČIGRE KOVALJEVSKE DO {T_SIMULATION_LONG:.0f} SEKUNDI / 5 MINUTA")
print("="*165)
print(df_master_300s.to_string(index=False))
print("="*165 + "\n")

print(f"✓ Svi rezultati su uspešno sačuvani u: '{BASE_OUT}/'")
print(f"  ├── 5-panel poređenja (0-4s):                       {DIR_MASTER_5PANEL}/")
print(f"  ├── Dugotrajni grafici (0-300s / 5 min):            {DIR_LONG_SIM}/")
print(f"  ├── Reprodukcija Figure 4 (Lott & Laws test):       {DIR_LOTT_TEST}/")
print(f"  └── Sačuvane tabele (4s, 300s i Lott Benchmark):   {DIR_TABLES}/\n")